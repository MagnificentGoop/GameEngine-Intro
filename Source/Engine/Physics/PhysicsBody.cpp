...
namespace bad
{
	PhysicsBody::PhysicsBody(const Transform2D& transform, const Vector2<float>& size, const PhysicsBodyDef& def, const Physics& physics) 
	{
		b2BodyDef bodyDef = b2DefaultBodyDef();

		// set body definition
		bodyDef.type = (def.isDynamic) ? b2_dynamicBody : b2_staticBody;
		bodyDef.position = bad::SceneConversions::ToB2(bad::SceneConversions::PixelToWorld(transform.position));
		...
		// create shape
		b2Vec2 hsize = bad::SceneConversions::ToB2(bad::SceneConversions::PixelToWorld(size * transform.scale * 0.5f));
		switch (def.shape)
		{
		case Shape::Box:
		{
	...
	{
		return bad::SceneConversions::WorldToPixel(bad::SceneConversions::ToVector2(b2Body_GetPosition(m_bodyId)));
	}

	void PhysicsBody::SetPosition(const Vector2<float>& position) const
	{
		b2Vec2 b2Position = bad::SceneConversions::ToB2(bad::SceneConversions::PixelToWorld(position));
	...
	void PhysicsBody::ApplyForce(const Vector2<float>& force) 
	{
		b2Body_ApplyForceToCenter(m_bodyId, bad::SceneConversions::ToB2(bad::SceneConversions::PixelToWorld(force)), true);
	}
	...
	void PhysicsBody::SetVelocity(const Vector2<float>& velocity) 
	{
		b2Body_SetLinearVelocity(m_bodyId, bad::SceneConversions::ToB2(bad::SceneConversions::PixelToWorld(velocity)));
	}

	Vector2<float> PhysicsBody::GetVelocity() 
	{
		return bad::SceneConversions::WorldToPixel(bad::SceneConversions::ToVector2(b2Body_GetLinearVelocity(m_bodyId)));
	}
}
