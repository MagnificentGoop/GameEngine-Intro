...
namespace bad {
	...
	void Box2DPhysicsComponent::SetVelocity(const Vector2<float>& velocity){
		m_physicsBody->SetVelocity(velocity);
	}
	Vector2<float> Box2DPhysicsComponent::GetVelocity(){
	...
	void Box2DPhysicsComponent::SetPosition(const Vector2<float>& position){
		m_physicsBody->SetPosition(position);
	}
	Vector2<float> Box2DPhysicsComponent::GetPosition() const{
